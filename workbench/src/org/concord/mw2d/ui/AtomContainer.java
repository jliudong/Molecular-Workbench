...
public class AtomContainer extends MDContainer implements RNATranscriptionListener, RNATranslationListener, MutationListener, ItemListener {
					...
					if ("READY".equalsIgnoreCase(s)) {
						...
						if ("".equals(s)) {
							...
							if (aa != null && aa.equalsIgnoreCase(s)) {
								...
								else if (Resource.get("AudioPlayer_java_zanting").equals(s)) {
	...
	private void createActions() {
		...
		xrayAction.putValue(Action.NAME, "X"+Resource.get("AtomContainer_java_shexian"));
		xrayAction.putValue(Action.SHORT_DESCRIPTION, "X"+Resource.get("AtomContainer_java_shexian"));
		...
		neutronAction.putValue(Action.NAME, Resource.get("AtomContainer_java_zhongzi"));
		neutronAction.putValue(Action.SHORT_DESCRIPTION, Resource.get("AtomContainer_java_zhongzi"));
		...
		stopDNAAction = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				dnaStepButton.setEnabled(true);
				dnaScroller.suspendSimulation();
				view.getActionMap().get(Resource.get("AudioPlayer_java_zanting")).actionPerformed(e);
			}
	...
	public void createDNAPanel() {
		...
		final Action stop = view.getActionMap().get(Resource.get("AudioPlayer_java_zanting"));
	...
	private class MB extends SimulatorMenuBar {
		...
		MB(MolecularModel m) {
			...
			menuItem.setText((s != null ? s : "X"+Resource.get("AtomContainer_java_shexian")) + "...");
			subMenu.add(menuItem);

			menuItem = new JMenuItem(neutronAction);
			s = getInternationalText("Neutron");
			menuItem.setText((s != null ? s : Resource.get("AtomContainer_java_zhongzi")) + "...");
			subMenu.add(menuItem);
			...
			menuItem = new JMenuItem((s != null ? s : Resource.get("JobTable_java_rewuguanliqi")) + "...");
			...
			energyTSItem = new JMenuItem(model.getActions().get(Resource.get("PageButtonMaker_java_xulie")));
