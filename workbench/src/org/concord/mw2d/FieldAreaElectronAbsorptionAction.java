 ...
 * 
 */
class FieldAreaElectronAbsorptionAction {

	private FieldArea area;
	private float scaleFactor = 100;

	public FieldAreaElectronAbsorptionAction(FieldArea area) {
		this.area = area;
	}

	JDialog createDialog(JComponent parent) {

		String s = MDView.getInternationalText("ElectronAbsorption");
		final JDialog dialog = new JDialog(JOptionPane.getFrameForComponent(parent), s != null ? s
				: "Electron Absorption", true);
		dialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);

		JPanel p = new JPanel(new BorderLayout());
		p.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
		int i = (int) (area.getElectronAbsorption() * scaleFactor);
		if (i < 0)
			i = 0;
		else if (i > 100)
			i = 100;
		...
		b.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				area.setElectronAbsorption(slider.getValue() / scaleFactor);
				dialog.dispose();
			}
		});
		p.add(b);
