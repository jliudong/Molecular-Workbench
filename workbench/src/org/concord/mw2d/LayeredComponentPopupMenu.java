...
class LayeredComponentPopupMenu extends JPopupMenu {
	...
	LayeredComponentPopupMenu(final MDView view) {
		...
		String s = MDView.getInternationalText(Resource.get("Editor_java_kaobei"));
		JMenuItem mi = new JMenuItem(s != null ? s : Resource.get("Editor_java_kaobei"), UserAction.getIcon(UserAction.DUPL_ID));
		...
		s = MDView.getInternationalText(Resource.get("Editor_java_jianqie"));
		if (s != null)
			mi.setText(s);
		add(mi);
