...
public abstract class MDView extends PrintableComponent {
	...
	final static String CUT = Resource.get("Editor_java_jianqie");
	final static String COPY = Resource.get("Editor_java_kaobei");
	final static String PASTE = Resource.get("Editor_java_zhantie");
	final static String CLEAR = "Clear";

	private final static String UNDO = Resource.get("Page_java_chexiao");
	private final static String REDO = Resource.get("Page_java_chongzuo");
	...
	public MDView() {
		...
		if (!MDContainer.isApplet()) {
			...
			ImageReader imageReader = new ImageReader(Resource.get("Page_java_youwenjianduru"), ModelerUtilities.fileChooser, this);
			...
			getInputMap().put((KeyStroke) imageReader.getValue(Action.ACCELERATOR_KEY), Resource.get("Page_java_youwenjianduru"));
			getActionMap().put(Resource.get("Page_java_youwenjianduru"), imageReader);
		...
		a.putValue(Action.SMALL_ICON, IconPool.getIcon(Resource.get("Page_java_chexiao")));
		...
		a.putValue(Action.SMALL_ICON, IconPool.getIcon(Resource.get("Page_java_chongzuo")));
		...
		a.putValue(Action.SHORT_DESCRIPTION, Resource.get("MDView_java_jiarezhileng"));
		...
		a.putValue(Action.SHORT_DESCRIPTION, Resource.get("MDView_java_reyu"));
		...
		a.putValue(Action.NAME, Resource.get("MDView_java_shijuexiaoguo"));
		a.putValue(Action.MNEMONIC_KEY, KeyEvent.VK_V);
		a.putValue(Action.SHORT_DESCRIPTION, Resource.get("MDView_java_shijuexiaoguo"));
		...
		a.putValue(Action.NAME, Resource.get("MDView_java_moxingshuxing"));
		a.putValue(Action.SHORT_DESCRIPTION, Resource.get("MDView_java_moxingshuxing"));
	...
	private void resizeTo(Dimension newSize, boolean isLoading, boolean fireUndoEvent) {
		...
		if (!getSize().equals(getPreferredSize())) {
			...
			if (!isLoading) {
				...
				if (fireUndoEvent) {
					getModel().getUndoManager().undoableEditHappened(new UndoableEditEvent(getModel(), new UndoableResizing(oldSize, newSize)));
	...
	public void setModel(MDModel m) {
		...
		a.putValue(Action.SMALL_ICON, IconPool.getIcon(Resource.get("Editor_java_jianqie")));
		a.putValue(Action.MNEMONIC_KEY, KeyEvent.VK_U);
		...
		a.putValue(Action.SMALL_ICON, IconPool.getIcon(Resource.get("Editor_java_zhantie")));
		...
		a.putValue(Action.SHORT_DESCRIPTION, Resource.get("MDView_java_qingchu"));
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				...
				if (undoUIComponent != null) {
					...
					undoUIComponent.setText(Resource.get("Page_java_chexiao"));
				}
				if (redoUIComponent != null) {
					redoUIComponent.setEnabled(false);
					redoUIComponent.setText(Resource.get("Page_java_chongzuo"));
				}
			}
		});
	}
	...
	void paintInfo(Graphics g) {
		if (getModel().isActionScriptRunning()) {
			...
			waitIcon.paintIcon(this, g, getWidth() - waitIcon.getIconWidth() - 5, getHeight() - waitIcon.getIconHeight() - 5);
		}
		if (showClock) {
			if (clockIcon == null)
		...
		if (backgroundImage != null) {
			...
			for (int i = 0; i < imax; i++) {
				for (int j = 0; j < jmax; j++) {
					if (backgroundImage != null)
						backgroundImage.paintIcon(null, g2, i * backgroundImage.getIconWidth(), j * backgroundImage.getIconHeight());
				}
			}
		}
	...
	public void setVelocityFlavor(VectorFlavor vf) {
		...
		if (vf.getWidth() != 1.0f || vf.getStyle() != LineStyle.STROKE_NUMBER_1) {
			velocityFlavor.setStroke(StrokeFactory.createStroke(vf.getWidth(), LineStyle.STROKES[vf.getStyle()].getDashArray()));
		}
	}
	...
	public void setMomentumFlavor(VectorFlavor vf) {
		momentumFlavor = vf;
		if (vf.getWidth() != 1.0f || vf.getStyle() != LineStyle.STROKE_NUMBER_1) {
			momentumFlavor.setStroke(StrokeFactory.createStroke(vf.getWidth(), LineStyle.STROKES[vf.getStyle()].getDashArray()));
		}
	}
	...
	public void setAccelerationFlavor(VectorFlavor vf) {
		...
		if (vf.getWidth() != 1.0f || vf.getStyle() != LineStyle.STROKE_NUMBER_1) {
			accelerationFlavor.setStroke(StrokeFactory.createStroke(vf.getWidth(), LineStyle.STROKES[vf.getStyle()].getDashArray()));
		}
	}
	...
	public void setForceFlavor(VectorFlavor vf) {
		forceFlavor = vf;
		if (vf.getWidth() != 1.0f || vf.getStyle() != LineStyle.STROKE_NUMBER_1) {
			forceFlavor.setStroke(StrokeFactory.createStroke(vf.getWidth(), LineStyle.STROKES[vf.getStyle()].getDashArray()));
		}
	}
	...
	boolean handleMouseDragged(int x, int y) {
		...
		if (selectedComponent instanceof LineComponent) {
			...
			if (rc.getSelectedHandle() >= 0) {
				...
				case RectangleComponent.UPPER_LEFT:
				case RectangleComponent.UPPER_RIGHT:
					rc.setRect(Math.min(x, anchorPoint.x), Math.min(y, anchorPoint.y), Math.abs(x - anchorPoint.x), Math.abs(y - anchorPoint.y));
					break;
				case RectangleComponent.TOP:
				case RectangleComponent.BOTTOM:
			...
			if (ec.getSelectedHandle() >= 0) {
				...
				case RectangleComponent.UPPER_LEFT:
				case RectangleComponent.UPPER_RIGHT:
					ec.setOval(Math.min(x, anchorPoint.x), Math.min(y, anchorPoint.y), Math.abs(x - anchorPoint.x), Math.abs(y - anchorPoint.y));
					break;
				case RectangleComponent.TOP:
				case RectangleComponent.BOTTOM:
	...
	public static abstract class State extends MDState {
		...
		private boolean showParticleIndex, showClock = true, showHeatBath = true, drawCharge = true, showMirrorImages = true, drawExternalForce;
		private boolean propertyDialogEnabled = true;
