...
class DefaultMesoPopupMenu extends JPopupMenu {
	...
	DefaultMesoPopupMenu(MesoView v) {
		...
		String s = MDView.getInternationalText(Resource.get("Editor_java_zhantie"));
		if (s != null)
			mi.setText(s);
		add(mi);
		addSeparator();
		...
		mi = new JMenuItem(s != null ? s : Resource.get("JobTable_java_rewuguanliqi"), IconPool.getIcon("taskmanager"));
