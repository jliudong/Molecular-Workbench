 ...
 * 
 */
class FieldAreaPhotonAbsorptionAction {

	private FieldArea area;
	private float scaleFactor = 100;

	public FieldAreaPhotonAbsorptionAction(FieldArea area) {
		this.area = area;
	}

	JDialog createDialog(JComponent parent) {

		String s = MDView.getInternationalText("PhotonAbsorption");
		final JDialog dialog = new JDialog(JOptionPane.getFrameForComponent(parent), s != null ? s
				: "Photon Absorption", true);
		dialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);

		JPanel p = new JPanel(new BorderLayout());
		p.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
		int i = (int) (area.getPhotonAbsorption() * scaleFactor);
		if (i < 0)
			i = 0;
		else if (i > 100)
			i = 100;
		...
		b.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				area.setPhotonAbsorption(slider.getValue() / scaleFactor);
				dialog.dispose();
			}
		});
		p.add(b);
