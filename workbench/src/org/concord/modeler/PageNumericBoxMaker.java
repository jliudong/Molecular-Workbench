...
class PageNumericBoxMaker extends ComponentMaker {
	...
	void invoke(Page page) {
		...
		if (pageNumericBox.isTargetClass()) {
			...
			if (pageNumericBox.modelID != -1) {
				...
				mc.getMdContainer().getModel().addModelListener(pageNumericBox);
			}
			else {
				Model m = (Model) modelComboBox.getSelectedItem();
				if (m != null) {
	...
	private void createContentPane() {
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Modeler.openWithNewInstance(pageNumericBox.getPage().getNavigator().getHomeDirectory()
						+ "tutorial/insertNumericBox.cml");
			}
		});
		p.add(button);
		...
		p.add(new JLabel(s != null ? s : Resource.get("TextBoxPanel_java_zitiyanse"), SwingConstants.LEFT));
		...
		p.add(new JLabel(s != null ? s : Resource.get("TextBoxPanel_java_zitixuanze"), SwingConstants.LEFT));
		...
		p.add(new JLabel(s != null ? s : Resource.get("ModelUtilities_java_zihao"), SwingConstants.LEFT));
