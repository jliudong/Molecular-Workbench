...
class BallsRenderer extends ShapeRenderer {
	...
	private void render3d() {
		...
		if (chargeFont3d == null)
			chargeFont3d = g3d.getFont3D("Courier New", Resource.get("FontDialog_java_jiacu"), JmolConstants.LABEL_DEFAULT_FONTSIZE + 2);
		minX = rectClip.x;
	...
	private void render(Atom atom) {
		...
		if (viewer instanceof ExtendedViewer) {
			if (atom.isGenericAtom()) {
				if (atom.colixAtom != atom.colixCustom && !viewer.keShading) {
