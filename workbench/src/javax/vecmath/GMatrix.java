...
public class GMatrix implements Serializable {
	 ...
	 * 
	 * Revision 1.13 1999/11/25 10:30:23 hiranabe get(GMatrix) bug
	 * 
	 * Revision 1.13 1999/11/25 10:30:23 hiranabe get(GMatrix) bug
	 * 
	 * Revision 1.12 1999/10/05 07:03:50 hiranabe copyright change
	 * 
	 * Revision 1.11 1999/03/04 09:16:33 hiranabe small bug fix and copyright
	 * change
	 * 
	 * Revision 1.10 1998/10/22 01:41:07 hiranabe add(GMatrix, GMatrix) method
	 * bug reported by kaneta@elelab.nsc.co.jp
	 * 
	 * Revision 1.9 1998/10/21 00:18:44 hiranabe GMatrix#mulTransposeRight bug
	 ...
	 * 
	 * Revision 1.5 1998/04/09 08:18:15 hiranabe minor comment change
	 * 
	 * Revision 1.4 1998/04/09 07:05:18 hiranabe API 1.1
	 * 
	 * Revision 1.3 1998/01/05 06:29:31 hiranabe copyright 98
	 * 
	 * Revision 1.2 1997/12/10 06:08:05 hiranabe added rank calculation to SVD
	 * toString '\n' -> "line.separator"
	 * 
	...
	/*
	 * Implementation note: The size of the matrix does NOT automaticly grow. It
	 * does only when setSize is called. I believe this is the spec and makes
	 * less confusion, less user bugs.
	 */
	...
	public final void mul(GMatrix m1, GMatrix m2) {
		...
		if (nRow != m1.nRow)
			throw new ArrayIndexOutOfBoundsException("nRow:" + nRow + " != m1.nRow:" + m1.nRow);
		if (nCol != m2.nCol)
			throw new ArrayIndexOutOfBoundsException("nCol:" + nCol + " != m2.nCol:" + m2.nCol);
		if (m1.nCol != m2.nRow)
	...
	public final void mul(GVector v1, GVector v2) {
		if (nRow < v1.getSize())
			throw new IllegalArgumentException("nRow:" + nRow + " < v1.getSize():" + v1.getSize());
		if (nCol < v2.getSize())
			throw new IllegalArgumentException("nCol:" + nCol + " < v2.getSize():" + v2.getSize());
	...
	public final void copySubMatrix(int rowSource, int colSource, int numRow, int numCol, int rowDest, int colDest,
		...
		for (int i = 0; i < numRow; i++)
			for (int j = 0; j < numCol; j++)
				target.elementData[(i + rowDest) * nCol + (j + colDest)] = elementData[(i + rowSource) * nCol
						+ (j + colSource)];
	}
	...
	public final void getRow(int row, double array[]) {
		...
		if (array.length < nCol)
			throw new ArrayIndexOutOfBoundsException("array length:" + array.length + " smaller than matrix's nCol:"
					+ nCol);
	...
	public final void getRow(int row, GVector vector) {
		...
		if (vector.getSize() < nCol)
			throw new ArrayIndexOutOfBoundsException("vector size:" + vector.getSize() + " smaller than matrix's nCol:"
					+ nCol);
		...
		for (int i = 0; i < nCol; i++) {
			...
			// System.arraycopy(elementData, row*nCol ,vector.elementData, 0,
			// nCol);
		}
	}
	...
	/**
	 * Places the values in the this matrix into the matrix m1; m1 should be at
	...
	public boolean epsilonEquals(GMatrix m1, float epsilon) {
		...
		for (int i = 0; i < nRow; i++)
			for (int j = 0; j < nCol; j++)
				if (epsilon < Math.abs(elementData[i * nCol + j] - m1.elementData[i * nCol + j]))
					return false;
