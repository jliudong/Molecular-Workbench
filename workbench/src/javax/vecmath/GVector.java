...
/**
 * A double precision, general, and dynamically resizeable one dimensional vector class. Index numbering begins with
 * zero.
 * 
...
public class GVector implements Serializable {
	/*
	 * $Log: GVector.java,v $ Revision 1.9 2006/07/28 17:01:32 qxie *** empty log message ***
	 * 
	 * Revision 1.6 2006/07/19 19:26:02 qxie *** empty log message ***
	 * 
	 * Revision 1.3 2006/06/20 14:00:03 qxie *** empty log message ***
	 * 
	 ...
	 * Revision 1.10 1999/10/05 07:03:50 hiranabe copyright change
	 * 
	 * Revision 1.9 1999/03/04 09:16:33 hiranabe small bug fix and copyright change
	 * 
	 ...
	 * 
	 * Revision 1.5 1998/04/10 04:52:14 hiranabe API1.0 -> API1.1 (added constructors, methods)
	 * 
	...
	/**
	 * Constructs a new generalized mathematic Vector with zero elements; length reprents the number of elements in the
	 * vector.
	 * 
	 * @param length
	...
	/**
	 * Constructs a new GVector by copying length elements from the array parameter. The parameter length must be less
	...
	public final void add(GVector vector) {
		...
		if (elementCount != vector.elementCount)
			throw new ArrayIndexOutOfBoundsException("this.size:" + elementCount + " != v2's size:"
					+ vector.elementCount);
	...
	public final void sub(GVector vector) {
		double[] v1data = vector.elementData;
		if (elementCount != vector.elementCount)
			throw new ArrayIndexOutOfBoundsException("this.size:" + elementCount + " != vector's size:"
					+ vector.elementCount);
	...
	public final void mul(GMatrix m1, GVector v1) {
		 ...
		 * 
		 * elementCount = m1.getNumRow(); if (elementData.length < elementCount) elementData = new double
		 * [elementCount]; // then above implementation.
		 */
	}
	...
	public final double getElement(int index) {
		try {
			...
			throw new ArrayIndexOutOfBoundsException("index:" + index + "must be in [0, " + (elementCount - 1) + "]");
		}
	}
	...
	/**
	 * Returns a hash number based on the data values in this object. Two different GMatrix objects with identical data
	 * values (ie, returns true for equals(GMatrix) ) will return the same hash number. Two objects with different data
	...
	/**
	 * Returns true if the Object o1 is of type GVector and all of the data members of t1 are equal to the corresponding
	 * data members in this GVector.
	 * 
	 * @param o1
	...
	public final void SVDBackSolve(GMatrix U, GMatrix W, GMatrix V, GVector b) {
		if (elementCount != U.getNumRow() || elementCount != U.getNumCol())
			throw new ArrayIndexOutOfBoundsException("this.size:" + elementCount + " != U.nRow,nCol:" + U.getNumRow()
					+ "," + U.getNumCol());
		if (elementCount != W.getNumRow())
			throw new ArrayIndexOutOfBoundsException("this.size:" + elementCount + " != W.nRow:" + W.getNumRow());
		...
		if (b.elementCount != V.getNumRow() || b.elementCount != V.getNumCol())
			throw new ArrayIndexOutOfBoundsException("b.size:" + elementCount + " != V.nRow,nCol:" + V.getNumRow()
					+ "," + V.getNumCol());
	 ...
	 * 
	 * @param LU
	 *            The matrix into which the lower and upper decompositions have been placed
	 * @param b
	...
	public final void LUDBackSolve(GMatrix LU, GVector b, GVector permutation) {
		...
		if (elementCount != b.elementCount)
			throw new ArrayIndexOutOfBoundsException("this.size:" + elementCount + " != b.size:" + b.elementCount);
		if (elementCount != LU.getNumRow())
			throw new ArrayIndexOutOfBoundsException("this.size:" + elementCount + " != LU.nRow:" + LU.getNumRow());
		if (elementCount != LU.getNumCol())
			throw new ArrayIndexOutOfBoundsException("this.size:" + elementCount + " != LU.nCol:" + LU.getNumCol());
